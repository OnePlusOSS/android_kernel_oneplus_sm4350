		...
		struct device_node *np, struct iris_cfg *pcfg)
{
	...
	key = "pxlw,iris-timing-switch-sequence-90";
	rc = iris_parse_optional_seq(np, key, &pcfg->timing_switch_seq_90hz);
	if (rc != 0) {
		IRIS_LOGE("%s(), failed to parse %s seq", __func__, key);
		return rc;
	}
...
void iris_send_timing_switch_pkt(void)
{
	...
	if ((refresh_rate == HIGH_FREQ) && (v_active == FHD_H || v_active == FHD_H_2))
		pseq = &pcfg->timing_switch_seq_1;

	if (refresh_rate == FREQ_90HZ)
		pseq = &pcfg->timing_switch_seq_90hz;
...
static uint32_t _iris_switch_case(const u32 refresh_rate, const u32 frame_height)
{
	...
	if ((frame_height == QHD_H) || (frame_height == QHD_H_2)) {
		pcfg->cmd_list_index = IRIS_DTSI0_PIP_IDX;
		if (cur_pt_mode)
			switch_mode = SWITCH_PT_TO_ABYP;
	}

	if (frame_height == FHD_H || frame_height == FHD_H_2) {
		pcfg->cmd_list_index = IRIS_DTSI1_PIP_IDX;
		if (cur_pt_mode) {
			if ((pcfg->cur_v_active == QHD_H) || (pcfg->cur_v_active == QHD_H_2))
				switch_mode = SWITCH_PT_TO_ABYP;
			else
...
static void _iris_switch_framerate(void)
{
	...
	if ((framerate == HIGH_FREQ)
			...
			|| pcfg->panel->cur_mode->timing.v_active == FHD_H_2))
		high = true;

	if (framerate == HIGH_FREQ)
		freq_90hz = true;

	iris_send_ipopt_cmds(IRIS_IP_SYS, freq_90hz ? 0xA2 : (high ? 0xA1:0xA0));
	udelay(100);
	iris_send_ipopt_cmds(IRIS_IP_RX, freq_90hz ? 0xE2 : (high ? 0xE1:0xE0));
	iris_send_ipopt_cmds(IRIS_IP_TX, freq_90hz ? 0x05 : (high ? 0x4:0x0));
	iris_set_out_frame_rate(framerate);
	iris_send_ipopt_cmds(IRIS_IP_RX, freq_90hz ? 0xF3 : (high ? 0xF2:0xF1));
#if defined(PXLW_IRIS_DUAL)
	iris_send_ipopt_cmds(IRIS_IP_RX_2, freq_90hz ? 0xF3 : (high ? 0xF2:0xF1));
	iris_send_ipopt_cmds(IRIS_IP_BLEND, freq_90hz ? 0xF2 : (high ? 0xF1:0xF0));
#endif
