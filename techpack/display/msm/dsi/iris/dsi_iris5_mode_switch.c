...
static u32 iris_get_vtotal(void)
{
	...
	else if (frc_setting->out_fps == FREQ_90HZ)
		payload = iris_get_ipopt_payload_data(IRIS_IP_DTG, 0x02, 2);
	else
...
static void iris_blending_timeout_set(int frc) {
	...
	if (framerate == FREQ_90HZ)
		freq_90hz = true;

	if (frc) {
		iris_send_ipopt_cmds(IRIS_IP_BLEND, freq_90hz ? 0x41 : (high? 0x31 : 0x11));	// blending: frc ocp
		iris_send_ipopt_cmds(IRIS_IP_BLEND, freq_90hz ? 0x40 : (high? 0x30 : 0x10));	// blending: frc
	} else {
