...
static int __init boot_mode_init_normal(void)
{
	...
	char *substr_recovery_reason = strnstr(boot_command_line,
		"androidboot.recoveryreason=", strlen(boot_command_line));
	char *substrftmstr = NULL;
	char *substrnormalstr = NULL;
	char *substrrecoveryreasonstr = NULL;

	substrftmstr = substrftm + strlen("androidboot.ftm_mode=");
	substrnormalstr = substrnormal + strlen("androidboot.mode=");
	substrrecoveryreasonstr = substr_recovery_reason + strlen("androidboot.recoveryreason=");
	...
	if(strncmp(substrrecoveryreasonstr, "bootloader_mode_select_recovery", 31) == 0)
		boot_mode = MSM_BOOT_MODE_RECOVERY;
