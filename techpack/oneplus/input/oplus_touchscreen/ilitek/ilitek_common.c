...
static int ilitek_read_limit_fw(struct seq_file *s, struct black_gesture_test *p, struct touchpanel_data *ts, struct auto_testdata *ilitek_testdata)
{
    ...
    if ((test_head->magic1 != Limit_MagicNum1) || (test_head->magic2 != Limit_MagicNum2)) {
        ...
        if (p) {
            snprintf(p->message, p->message_size - 1, "limit image is not generated by oplus\n");
        }
        return  -1;
    }
...
static int ilitek_auto_test_item(struct seq_file *s, struct touchpanel_data *ts, struct auto_testdata *ilitek_testdata)
{
    int error_count = 0;
    ...
    ilitek_test_ops = (struct ilitek_test_operations *)com_test_data_p->chip_test_ops;
    if (!ilitek_test_ops->auto_test_preoperation) {
        TPD_INFO("not support gd_test_ops->auto_test_preoperation callback\n");
    } else {
        error_count += ilitek_test_ops->auto_test_preoperation(s, ts->chip_data, ilitek_testdata);
    }
END:
    return error_count;
}
...
//step2: test support item
static int ilitek_black_test_item(struct black_gesture_test *p, struct touchpanel_data *ts, struct auto_testdata *ilitek_testdata)
{
    int error_count = 0;
    struct ilitek_test_operations *ilitek_test_ops = NULL;
    struct com_test_data *com_test_data_p = NULL;
    ...
    if (!ilitek_test_ops->black_screen_preoperation) {
        ...
        error_count += ilitek_test_ops->black_screen_preoperation(p->message, p->message_size, ts->chip_data, ilitek_testdata);
    }
END:
    if (p->message_size >= 20) {
        snprintf(p->message, 20, "%d errors. %s", error_count, error_count ? "" : "All test passed.");
    }
...
END:
    seq_printf(s, "imageid = 0x%llx, deviceid = 0x%llx\n", ilitek_testdata.TP_FW, ilitek_testdata.DEV_TP_FW);
