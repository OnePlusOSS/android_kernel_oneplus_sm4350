    ...
    for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
        if (fbi[i].mode == mode && fbi[i].len != 0) {
            ...
            ILI_INFO("%s CRC is %s hex(%x) : dma(%x) : iram(%x), calculation len is 0x%x\n",
                     fbi[i].name, ((crc != dma)
                                   || (iram_crc_err)) ? "Invalid !" : "Correct !", crc, dma, iram_crc, fbi[i].len - crc_len);
...
static int ilitek_tddi_fw_update_block_info(u8 *pfw) {
    ...
    fbi[DATA].mem_start = (fbi[DATA].fix_mem_start != INT_MAX) ? fbi[DATA].fix_mem_start :
                          DLM_START_ADDRESS;
    ...
    ILI_INFO("report_info_block : nIsI2C = %d, nReserved00 = %d, nReserved01 = %x, nReserved02 = %x,  nReserved03 = %x\n",
             ilits->rib.nIsI2C, ilits->rib.nReserved00, ilits->rib.nReserved01, ilits->rib.nReserved02,
...
static int ilitek_tddi_fw_ili_convert(u8 *pfw) {
    ...
    if (!ilits->p_firmware_headfile->data || ilits->p_firmware_headfile->size <= ILI_FILE_HEADER
