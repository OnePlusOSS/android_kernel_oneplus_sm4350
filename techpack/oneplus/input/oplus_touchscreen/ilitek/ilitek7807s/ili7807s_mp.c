...
#if 0
            p_mp_test_items->bch_mrk_max = (s32 **)kzalloc(p_mp_test_items->bch_mrk_frm_num * sizeof(s32 *),
                                                           GFP_KERNEL);

            if (ERR_ALLOC_MEM(p_mp_test_items->bch_mrk_max)) {
                ILI_ERR("Failed to allocate bch_mrk_max mem\n");
                return -ENOMEM;
            }

            p_mp_test_items->bch_mrk_min = (s32 **)kzalloc(p_mp_test_items->bch_mrk_frm_num * sizeof(s32 *),
                                                           GFP_KERNEL);

            if (ERR_ALLOC_MEM(p_mp_test_items->bch_mrk_min)) {
                ILI_ERR("Failed to allocate bch_mrk_min mem\n");
                ili_kfree((void **)&p_mp_test_items->bch_mrk_max);
                return -ENOMEM;
            }
    ...
    if (!p_test_item_info) {
        ...
        if (p_test_item_info->p_buffer[0] == 1 && p_test_item_info->para_num >= 50) {
            ...
            for (j = 0 ; j < (ARRAY_SIZE(p_open_test->cap1_raw_cmd)) ; j ++) {
                p_open_test->cap1_raw_cmd[j] = (u8) p_test_item_info->p_buffer[j + 1 + ARRAY_SIZE(
            ...
            p_mp_test_items->gain = p_test_item_info->p_buffer[47];
...
static void mp_print_csv_cdc_cmd(char *csv, int *csv_len, int index, int file_size) {
    ...
    if (ipio_strcmp(name, "open test_c") == 0) {
        ...
        for (i = 0; i < size; i++) {
            if (0 == i) {
                memcpy(open_c_cmd_data,  core_mp->open_para.cap1_dac_cmd, sizeof(open_c_cmd_data));
            } else {
                memcpy(open_c_cmd_data,  core_mp->open_para.cap1_raw_cmd, sizeof(open_c_cmd_data));
            }
    ...
    for (y = 0; y < core_mp->ych_len; y++) {
        ...
        for (x = 0; x < core_mp->xch_len; x++) {
            ...
            if ((tmp[shift] <= max_ts[shift] && tmp[shift] >= min_ts[shift]) || (type != TYPE_JUGE)) {
                if ((tmp[shift] == COMPARE_MAX || tmp[shift] == COMPARE_MIN) && (type == TYPE_BENCHMARK)) {
                    DUMP("%s", "BYPASS,");
...
static s32 open_c_formula(int inCap1DAC, int inCap1Raw, int accuracy) {
    ...
    int inVadc_range = 36;
    int inVbk = 39;
    int inCbk_step = core_mp->open_para.cbk_step;/* from mp.ini */
    int inCint = core_mp->open_para.cint;/* from mp.ini */
    int inVdrv = core_mp->open_para.tvch - core_mp->open_para.tvcl;/* from mp.ini */
    int inGain = core_mp->open_para.gain;/*  from mp.ini */
...
static int codeToOhm(s32 *ohm, s32 *Code, u16 *v_tdf, u16 *h_tdf) {
    ...
    u16 id = core_mp->chip_id;
    u8 type = core_mp->chip_type;
    int inTVCH = core_mp->short_para.tvch;
    int inTVCL = core_mp->short_para.tvcl;
    int inVariation = core_mp->short_para.variation;
    int inTDF1 = 0;
    int inTDF2 = 0;
    int inCint = core_mp->short_para.cint;
    int inRinternal = core_mp->short_para.rinternal;
    s32 temp = 0;
    int j = 0;
...
static int short_test(int index, int frame_index) {
    ...
    ILI_INFO("TVCH = %d, TVCL = %d, Variation = %d, V_TDF1 = %d, V_TDF2 = %d, H_TDF1 = %d, H_TDF2 = %d, Cint = %d, Rinternal = %d\n"
             , core_mp->short_para.tvch, core_mp->short_para.tvcl, core_mp->short_para.variation,
             tItems[index].v_tdf_1, tItems[index].v_tdf_2, tItems[index].h_tdf_1, tItems[index].h_tdf_2,
...
static int allnode_open_cdc_data(int mode, int *buf) {
    ...
    ili_dump_data(buf, 10, core_mp->frame_len,  core_mp->xch_len, "Open CDC combined");
out:
    ili_kfree((void **)&ori);
    return ret;
}
...
static int mp_get_timing_info(void *chip_data, struct auto_testdata *testdata) {
    ...
    if (!p_test_item_info) {
        ...
        if (p_test_item_info->p_buffer[0] == 1) {
            core_mp->isLongV = p_test_item_info->p_buffer[7];
...
static int mp_show_result(void *chip_data, struct auto_testdata *testdata, bool lcm_on) {
    int ret = MP_DATA_PASS;
    int i, x, y, j, k, csv_len = 0, pass_item_count = 0, line_count = 0, get_frame_cont = 1;
    s32 *max_threshold = NULL, *min_threshold = NULL;
    char *csv = NULL;
    struct core_mp_test_data *core_mp = &ilits->core_mp;
    ...
    if (lcm_on && testdata->fp) {
        ...
        tp_test_write(testdata->bs_fp, testdata->bs_length, csv, csv_len, testdata->bs_pos);
    }
...
static void mp_test_run(bool lcm_on, int test_index, struct auto_testdata *testdata) {
    int i = 0;
    struct core_mp_test_data *core_mp = &ilits->core_mp;
    ...
    if (strncmp("open test_c", tItems[i].desp, strlen("open test_c")) == 0) {
        ilitek_get_para_openshort(testdata, &tItems[i], &core_mp->open_para, tItems[i].test_index);
    } else {
        ilitek_get_item_para(testdata, &tItems[i], tItems[i].test_index);
    }

    if (!tItems[i].run) {
        ILI_ERR("Not found test_index : %d lcm_on = %d, run = %d\n", test_index, lcm_on, tItems[i].run);
        return;
    }
    ...
    if (tItems[i].catalog == PEAK_TO_PEAK_TEST) {
        if (tItems[i].max_min_mode == 1 && tItems[i].spec_option == 1) {
            ...
            for (j = 1; j < 11; j++) {
                ...
                snprintf(str, sizeof(str), "%s%s%s%d", tItems[i].desp, "_", BENCHMARK_KEY_NAME, j);
...
#if 0
            ili_kfree((void **)&tItems[i].bch_mrk_max);
            ili_kfree((void **)&tItems[i].bch_mrk_min);
...
static int ili7807_black_screen_preoperation(char *msg, int msg_size, void *chip_data,
                                             struct auto_testdata *testdata) {
    //char apk_ret[100] = {0};
    struct ilitek_ts_data *chip_info = (struct ilitek_ts_data *)chip_data;
    mutex_lock(&chip_info->touch_mutex);
