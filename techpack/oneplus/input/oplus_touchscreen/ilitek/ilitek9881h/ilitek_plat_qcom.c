...
static int core_spi_read_data_after_checksize(void *chip_data, uint8_t *data, int size)
{
    int ret = 0;
    struct ilitek_tddi_dev *chip_info = (struct ilitek_tddi_dev *)chip_data;
...
static int core_spi_setup(void *chip_data, u32 freq)
{
    ...
    chip_info->spi->chip_select = 0; //modify reg=0 for more tp vendor share same spi interface
...
static void ilitek_tddi_touch_esd_gesture_iram(void *chip_data)
{
    ...
    if (ilitek_ice_mode_write(chip_info, SPI_ESD_GESTURE_PWD_ADDR, ESD_GESTURE_PWD, 4) < 0)
...
static int ilitek_tddi_get_tp_recore_data(void *chip_data, u16 *out_buf, u32 out_len)
{
    u8 buf[8] = {0};
    u8 record_case = 0;
    s8 index;
...
static void ilitek_tddi_touch_send_debug_data(void *chip_data, u8 *buf, int len)
{
    ...
    if (chip_info->debug_node_open && !(ERR_ALLOC_MEM(chip_info->debug_buf))
        ...
        memset(chip_info->debug_buf[chip_info->debug_data_frame], 0x00, (u8)sizeof(u8) * DEBUG_DATA_MAX_LENGTH);
        tp_memcpy(chip_info->debug_buf[chip_info->debug_data_frame], (u8)sizeof(u8) * DEBUG_DATA_MAX_LENGTH,
                  buf, len, len);
...
static int ilitek_tddi_report_handler(void *chip_data)
{
    ...
    if (rlen < 0 || rlen >= 2048) {
        ...
        if (rlen == DO_SPI_RECOVER && chip_info->actual_tp_mode == P5_X_FW_GESTURE_MODE && chip_info->gesture) {
...
static u32 ilitek_trigger_reason(void *chip_data, int gesture_enable, int is_suspended)
{
    int ret = 0;
    struct ilitek_tddi_dev *chip_info = (struct ilitek_tddi_dev *)chip_data;
...
static int ilitek_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
    ...
    gesture->Point_start.x = gesture->Point_start.x * chip_info->panel_wid / TPD_WIDTH;
    gesture->Point_start.y = gesture->Point_start.y * chip_info->panel_hei / TPD_HEIGHT;
                                        ...
                                        bool force)
{
    ...
    if (ret < 0) {
        TPD_INFO("Failed to upgrade firmware, ret = %d\n", ret);
        tp_healthinfo_report(monitor_data, HEALTH_FW_UPDATE, "Failed to upgrade firmware");
        return ret;
    }
...
static int ilitek_alloc_global_data(void)
{
    ...
    idev->core_mp = (struct core_mp_test_data *)kzalloc(sizeof(struct core_mp_test_data), GFP_KERNEL);
