...
static int debug_mode_get_data(void *chip_data, struct file_buffer *file, u8 type, u32 frame_count)
{
    int ret;
    int timeout = 50;
    u8 cmd[2] = { 0 }, row, col;
    ...
    while ((write_index < frame_count) && (timeout > 0)) {
        TPD_INFO("frame = %d,index = %d,count = %d\n", write_index, write_index % 1024, chip_info->debug_data_frame);
...
static ssize_t ilitek_proc_get_delta_data_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
    ...
    memset(chip_info->g_user_buf, 0, chip_info->g_user_buf_size * sizeof(unsigned char));
    ...
    size = snprintf(chip_info->g_user_buf + size, PAGE_SIZE - size, "======== Deltadata ========\n");
    TPD_INFO("======== Deltadata ========\n");
    ...
    // print delta data
    for (y = 0; y < row; y++) {
        size += snprintf(chip_info->g_user_buf + size, PAGE_SIZE - size, "[%2d] ", (y + 1));
        TPD_INFO("[%2d] ", (y + 1));

        for (x = 0; x < col; x++) {
            int shift = y * col + x;
            size += snprintf(chip_info->g_user_buf + size, PAGE_SIZE - size, "%5d", delta[shift]);
...
static ssize_t ilitek_proc_fw_get_raw_data_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
    ...
    size = snprintf(chip_info->g_user_buf, PAGE_SIZE, "======== RawData ========\n");
    TPD_INFO("======== RawData ========\n");
    ...
    for (y = 0; y < row; y++) {
        ...
        for (x = 0; x < col; x++) {
            int shift = y * col + x;
            size += snprintf(chip_info->g_user_buf + size, PAGE_SIZE - size, "%5d", rawdata[shift]);
...
static ssize_t ilitek_proc_rw_tp_reg_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
    ...
    if (type == read) {
        ...
        size = snprintf(chip_info->g_user_buf, PAGE_SIZE, "READ:addr = 0x%06x, read = 0x%08x\n", addr, read_data);
    } else {
        ...
        size = snprintf(chip_info->g_user_buf, PAGE_SIZE, "WRITE:addr = 0x%06x, write = 0x%08x, len =%d byte\n", addr, write_data, write_len);
    }
...
static ssize_t ilitek_proc_debug_switch_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
    int ret = 0;
    int i = 0;
    struct ilitek_tddi_dev *chip_info = PDE_DATA(file_inode(filp));
    ...
    if (chip_info->debug_node_open) {
        if (ERR_ALLOC_MEM(chip_info->debug_buf)) {
            chip_info->debug_buf = (unsigned char **)kmalloc(1024  * sizeof(unsigned char *), GFP_KERNEL);
            if (!ERR_ALLOC_MEM(chip_info->debug_buf)) {
                for (i = 0; i < 1024; i++) {
                    chip_info->debug_buf[i] = (char *)kmalloc(2048 * sizeof(unsigned char), GFP_KERNEL);
...
static ssize_t ilitek_proc_debug_message_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
    ...
    if (ERR_ALLOC_MEM(tmpbuf)) {
        ...
        ret = copy_to_user(buff, tmpbufback, send_data_len); /*chip_info->debug_buf[0] */
        goto out;
    }
    ...
    if (chip_info->debug_data_frame > 0) {
        ...
        if (p == 5 || size == (DEBUG_DATA_MAX_LENGTH * 2) || size == DEBUG_DATA_MAX_LENGTH) {
            ...
            for (i = 1; i <= chip_info->debug_data_frame; i++)
                memcpy(chip_info->debug_buf[i - 1], chip_info->debug_buf[i], DEBUG_DATA_MAX_LENGTH);
        }
    } else {
...
static ssize_t ilitek_proc_get_debug_mode_data_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
    ...
    ret = debug_mode_get_data(chip_info, &csv, P5_X_FW_RAW_DATA_MODE, chip_info->raw_count);
    if (ret < 0)
        goto out;
    ...
    csv.file_len += snprintf(csv.ptr + csv.file_len,
                             ...
                             "\n\n=======Delta data=======");
    file_write(&csv, false);
    ret = debug_mode_get_data(chip_info, &csv, P5_X_FW_DELTA_DATA_MODE, chip_info->delta_count);
    if (ret < 0)
        goto out;
...
static ssize_t ilitek_node_ioctl_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
    ...
    if (strcmp(cmd, "hwreset") == 0) {
        ...
        ilitek_tddi_ic_func_ctrl(chip_info, "gesture_demo_en", chip_info->gesture_demo_en);
...
void ilitek_tddi_node_init(void *chip_data)
{
    ...
    for (; i < ARRAY_SIZE(proc_table); i++) {
        proc_table[i].node = proc_create_data(proc_table[i].name, 0644, chip_info->proc_dir_ilitek, proc_table[i].fops, chip_info);
