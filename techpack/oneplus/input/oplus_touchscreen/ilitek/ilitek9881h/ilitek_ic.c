...
int ilitek_ice_mode_ctrl(void *chip_data, bool enable, bool mcu)
{
    ...
    if (enable) {
        ...
        do {
            ...
            ret = ilitek_ice_mode_read(chip_info, chip_info->chip->pid_addr, &pid, sizeof(u32));
            if (ret < 0) {
                continue;
            }
...
int ilitek_tddi_ic_watch_dog_ctrl(void *chip_data, bool write, bool enable)
{
    ...
    if (!write) {
        ret = ilitek_ice_mode_read(chip_info, chip_info->chip->wdt_addr, &reg_data, sizeof(u8));
        TPD_INFO("Read WDT: %s\n", (reg_data ? "ON" : "OFF"));
        return reg_data;
    }

    TPD_INFO("%s WDT, key = %x\n", (enable ? "Enable" : "Disable"), chip_info->chip->wtd_key);

    if (enable) {
        ilitek_ice_mode_write(chip_info, chip_info->chip->wdt_addr, 1, 1);
    } else {
        ilitek_ice_mode_write(chip_info, chip_info->chip->wdt_addr, (chip_info->chip->wtd_key & 0xff), 1);
        ilitek_ice_mode_write(chip_info, chip_info->chip->wdt_addr, (chip_info->chip->wtd_key >> 8), 1);
        /* need to delay 300us after stop mcu to wait fw relaod */
        udelay(300);
    }

    while (timeout > 0) {
        ret = ilitek_ice_mode_read(chip_info, TDDI_WDT_ACTIVE_ADDR, &reg_data, sizeof(u8));
...
void ilitek_tddi_ic_set_ddi_reg_onepage(void *chip_data, u8 page, u8 reg, u8 data)
{
    int wdt;
...
u32 ilitek_tddi_ic_get_pc_counter(void *chip_data)
{
    ...
    ilitek_ice_mode_read(chip_info, chip_info->chip->pc_counter_addr, &pc, sizeof(u32));
...
int ilitek_tddi_ic_get_fw_ver(void *chip_data)
{
    ...
    if (chip_info->ts->panel_data.manufacture_info.version) {
        ...
        if (chip_info->ts->panel_data.vid_len == 0) {
            ...
            strlcpy(&(chip_info->ts->panel_data.manufacture_info.version[12]), dev_version, 3);
        } else {
...
out:
    ...
    TPD_INFO("Panel info: width = %d, height = %d\n", chip_info->panel_wid, chip_info->panel_hei);
    return ret;
}
...
int ilitek_tddi_ic_get_info(void *chip_data)
{
    ...
    ilitek_ice_mode_read(chip_info, chip_info->chip->pid_addr, &(chip_info->chip->pid), sizeof(u32));
    ...
    ilitek_ice_mode_read(chip_info, chip_info->chip->otp_addr, &(chip_info->chip->otp_id), sizeof(u32));
    ilitek_ice_mode_read(chip_info, chip_info->chip->ana_addr, &(chip_info->chip->ana_id), sizeof(u32));
    ...
    ret = ilitek_tddi_ic_check_support(chip_info, chip_info->chip->pid, chip_info->chip->id);
    return ret;
}
