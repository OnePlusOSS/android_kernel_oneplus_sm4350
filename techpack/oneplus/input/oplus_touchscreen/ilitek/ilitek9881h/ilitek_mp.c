...
static int  ilitek_get_item_para(struct auto_testdata *testdata, struct mp_test_items *p_mp_test_items, int item_index)
{
    ...
    if (!p_test_item_info) {
        ...
        if (p_test_item_info->item_limit_type == LIMIT_TYPE_TX_RX_DATA) {
            p_mp_test_items->bench_mark_max = (uint32_t *)(testdata->fw->data + p_test_item_info->top_limit_offset);
            p_mp_test_items->bench_mark_min = (uint32_t *)(testdata->fw->data + p_test_item_info->floor_limit_offset);
        }
    ...
    if (!p_test_item_info) {
        ...
        if (p_test_item_info->p_buffer[0] == 1 && p_test_item_info->para_num >= 50) {
            ...
            for (j = 0 ; j < (ARRAY_SIZE(p_open_test->cap1_raw_cmd)) ; j ++) {
                p_open_test->cap1_raw_cmd[j] = (u8) p_test_item_info->p_buffer[j + 1 + ARRAY_SIZE(p_open_test->cap1_dac_cmd)];
            }
            ...
            TPD_DEBUG("cap1_dac_cmd:%*ph\n", sizeof(p_open_test->cap1_dac_cmd), p_open_test->cap1_dac_cmd);
            TPD_DEBUG("cap1_raw_cmd:%*ph\n", sizeof(p_open_test->cap1_raw_cmd), p_open_test->cap1_raw_cmd);
        }
        if (p_test_item_info->item_limit_type == LIMIT_TYPE_TX_RX_DATA) {
            p_mp_test_items->bench_mark_max = (uint32_t *)(testdata->fw->data + p_test_item_info->top_limit_offset);
            p_mp_test_items->bench_mark_min = (uint32_t *)(testdata->fw->data + p_test_item_info->floor_limit_offset);
...
static void mp_print_csv_header(void *chip_data, char *csv, int *csv_len, int *csv_line)
{
    int i;
    int tmp_len = *csv_len;
...
static void mp_print_csv_cdc_cmd(void *chip_data, char *csv, int *csv_len, int index)
{
    int tmp_len = *csv_len;
    char str[128] = {0};
    int i, j,  j_len = 0;
    ...
    if (strncmp(name, "open test_c", strlen(name)) == 0) {
        for (i = 0; i < ARRAY_SIZE(open_c_cmd); i++) {
            if (0 == i) {
                memcpy(open_c_cmd_data,  core_mp->open_c_spec.cap1_dac_cmd, sizeof(open_c_cmd_data));
            } else {
                memcpy(open_c_cmd_data,  core_mp->open_c_spec.cap1_raw_cmd, sizeof(open_c_cmd_data));
            }
            j_len = 0;
            memset(str, 0, sizeof(str));
            for (j = 0; j < ARRAY_SIZE(open_c_cmd_data); ) {
                j_len += snprintf(str + j_len, sizeof(str) - j_len, "0x%x,", open_c_cmd_data[j++]);
            }
            tmp_len += snprintf(csv + tmp_len, CSV_FILE_SIZE - tmp_len,
                                "%s = ,%s\n", open_c_cmd[i], str);
        }
        ...
        for (j = 0; j < ARRAY_SIZE(tItems[index].p5v_cmd)  ; ) {
            j_len += snprintf(str + j_len, sizeof(str) - j_len, "0x%x,", tItems[index].p5v_cmd[j++]);
        }
        tmp_len += snprintf(csv + tmp_len, CSV_FILE_SIZE - tmp_len, "CDC command = ,%s\n", str);
    }
    *csv_len = tmp_len;
}

static void mp_compare_cdc_show_result(void *chip_data, int index, s32 *tmp, char *csv,
...
void allnode_open_cdc_result(void *chip_data, int index, int *buf, int *dac, int *raw)
{
    int i;
...
static int short_test(void *chip_data, int index, int frame_index)
{
    ...
    if (core_mp->protocol_ver >= PROTOCOL_VER_540) {
        ...
        for (j = 0; j < core_mp->frame_len; j++)
            tItems[index].buf[frame_index * core_mp->frame_len + j] = codeToOhm(chip_info, core_mp->frame_buf[j], v_tdf, h_tdf);
    } else {
        for (j = 0; j < core_mp->frame_len; j++)
            tItems[index].buf[frame_index * core_mp->frame_len + j] = core_mp->frame_buf[j];
    }
...
static int allnode_mutual_cdc_data(void *chip_data, int index)
{
    ...
    for (i = 0; i < core_mp->frame_len; i++) {
        if (tItems[index].cmd == CMD_MUTUAL_DAC) {
            ...
            core_mp->frame_buf[i] = (inDACp + inDACn) / 2;
        } else {
            /* H byte + L byte */
    ...
    ilitek_dump_data(core_mp->frame_buf, 32, core_mp->frame_len,   core_mp->xch_len, "Mutual CDC combined");
...
static int create_mp_test_frame_buffer(void *chip_data, int index, int frame_count)
{
    struct ilitek_tddi_dev *chip_info = (struct ilitek_tddi_dev *)chip_data;
    ...
    if (tItems[index].buf == NULL) {
        tItems[index].buf = vmalloc(frame_count * chip_info->core_mp->frame_len * sizeof(s32));
    ...
    if (tItems[index].result_buf == NULL) {
        tItems[index].result_buf = kcalloc(chip_info->core_mp->frame_len, sizeof(s32), GFP_KERNEL);
    ...
    if (tItems[index].max_buf == NULL) {
        tItems[index].max_buf = kcalloc(chip_info->core_mp->frame_len, sizeof(s32), GFP_KERNEL);
    ...
    if (tItems[index].min_buf == NULL) {
        tItems[index].min_buf = kcalloc(chip_info->core_mp->frame_len, sizeof(s32), GFP_KERNEL);
    ...
    for (i = 0; i < get_frame_cont; i++) {
        ...
        default:
            for (j = 0; j < chip_info->core_mp->frame_len; j++)
                tItems[index].buf[i * chip_info->core_mp->frame_len + j] = chip_info->core_mp->frame_buf[j];
            break;
        }
        compare_MaxMin_result(chip_info, index, &tItems[index].buf[i * chip_info->core_mp->frame_len]);
    }
...
static int open_test_cap(void *chip_data, int index)
{
    ...
    if (tItems[index].spec_option == BENCHMARK) {
        if (1)
            dump_benchmark_data(chip_info, tItems[index].bench_mark_max, tItems[index].bench_mark_min);
    }
    ...
    for (i = 0; i < tItems[index].frame_count; i++) {
        ...
        allnode_open_cdc_result(chip_info, index, open[i].dcl_cap, open[i].cap_dac, open[i].cap_raw);
        ...
        compare_MaxMin_result(chip_info, index, &tItems[index].buf[i * core_mp->frame_len]);
    }
...
static int mp_test_data_sort_average(void *chip_data, s32 *oringin_data, int index, s32 *avg_result)
{
    int i, j, k, x, y, len = 5;
    s32 u32temp;
    ...
    u32data_buff = kcalloc(chip_info->core_mp->frame_len * tItems[index].frame_count, sizeof(s32), GFP_KERNEL);
    u32sum_raw_data = kcalloc(chip_info->core_mp->frame_len, sizeof(s32), GFP_KERNEL);
    if (ERR_ALLOC_MEM(u32sum_raw_data) || (ERR_ALLOC_MEM(u32data_buff))) {
        TPD_INFO("Failed to allocate u32sum_raw_data FRAME buffer\n");
        return -ENOMEM;
    }

    for (i = 0; i < chip_info->core_mp->frame_len * tItems[index].frame_count; i++) {
...
static void mp_compare_cdc_result(void *chip_data, int index, s32 *tmp, s32 *max_ts, s32 *min_ts, int *result)
{
    int i;
    struct ilitek_tddi_dev *chip_info = (struct ilitek_tddi_dev *)chip_data;
...
static int mp_comp_result_before_retry(void *chip_data, int index)
{
    ...
    if (tItems[index].trimmed_mean && tItems[index].catalog != PEAK_TO_PEAK_TEST) {
        mp_test_data_sort_average(chip_info, tItems[index].buf, index, tItems[index].result_buf);
        mp_compare_cdc_result(chip_info, index, tItems[index].result_buf, max_threshold, min_threshold, &test_result);
    } else {
        mp_compare_cdc_result(chip_info, index, tItems[index].buf, max_threshold, min_threshold, &test_result);
        mp_compare_cdc_result(chip_info, index, tItems[index].buf, max_threshold, min_threshold, &test_result);
    }
...
static void mp_show_result(void *chip_data, struct auto_testdata *testdata)
{
    ...
    for (i = 0; i < ARRAY_SIZE(tItems); i++) {
        ...
        if (tItems[i].spec_option == BENCHMARK) {
            ...
            mp_compare_cdc_show_result(chip_info, i, tItems[i].bench_mark_max, csv, &csv_len, TYPE_BENCHMARK, max_threshold, min_threshold, "Max_Bench");
            mp_compare_cdc_show_result(chip_info, i, tItems[i].bench_mark_min, csv, &csv_len, TYPE_BENCHMARK, max_threshold, min_threshold, "Min_Bench");
        } else {
            for (j = 0; j < chip_info->core_mp->frame_len; j++) {
        ...
        if (strcmp(tItems[i].name, "open test_c") == 0) {
            mp_compare_cdc_show_result(chip_info, i, core_mp->cap_dac, csv, &csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, "CAP_DAC");
            mp_compare_cdc_show_result(chip_info, i, core_mp->cap_raw, csv, &csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, "CAP_RAW");
        }
        ...
        /* general result */
        if (tItems[i].trimmed_mean && tItems[i].catalog != PEAK_TO_PEAK_TEST) {
            mp_compare_cdc_show_result(chip_info, i, tItems[i].result_buf, csv, &csv_len, TYPE_JUGE, max_threshold, min_threshold, "Mean result");
        } else {
            mp_compare_cdc_show_result(chip_info, i, tItems[i].max_buf, csv, &csv_len, TYPE_JUGE, max_threshold, min_threshold, "Max Hold");
            mp_compare_cdc_show_result(chip_info, i, tItems[i].min_buf, csv, &csv_len, TYPE_JUGE, max_threshold, min_threshold, "Min Hold");
        }
        ...
        for (j = 0; j < get_frame_cont; j++) {
            ...
            mp_compare_cdc_show_result(chip_info, i, &tItems[i].buf[(j * chip_info->core_mp->frame_len)], csv, &csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, frame_name);
        }
    }
    ...
    vfs_write(testdata->fp, csv, csv_len, &testdata->pos);
...
static void mp_test_run(void *chip_data, char *item, struct auto_testdata *testdata)
{
    int i;
    struct ilitek_tddi_dev *chip_info = (struct ilitek_tddi_dev *)chip_data;

    if (item == NULL || strncmp(item, " ", strlen(item)) == 0 || chip_info->core_mp->frame_len == 0) {
        chip_info->core_mp->final_result = MP_FAIL;
        TPD_INFO("Invaild string (%s) or frame length (%d)\n", item, chip_info->core_mp->frame_len);
        return;
    }
    ...
    for (i = 0; i < MP_TEST_ITEM; i++) {
        if (strncmp(item, tItems[i].desp, strlen(item)) == 0) {
            ...
            if (strncmp("open test_c", tItems[i].desp, strlen("open test_c")) == 0) {
                ilitek_get_para_openshort(testdata, &tItems[i], &chip_info->core_mp->open_c_spec, tItems[i].test_index);
            } else {
...
static void mp_copy_ret_to_apk(void *chip_data, char *buf, struct seq_file *s, char *message, int msg_size)
{
    int i, run = 0, count = 0;
...
static int mp_get_timing_info(void *chip_data, struct auto_testdata *testdata)
{
    ...
    if (!p_test_item_info) {
        ...
        if (p_test_item_info->p_buffer[0] == 1) {
            chip_info->core_mp->isLongV = p_test_item_info->p_buffer[7];
        }
    }
    TPD_INFO("p_test_item_info->p_buffer[7]%d\n", p_test_item_info->p_buffer[7]);
    TPD_INFO("DDI Mode = %s\n", (chip_info->core_mp->isLongV ? "Long V" : "Long H"));
                             ...
                             struct auto_testdata *testdata)
{
    ...
    if (chip_info->protocol->ver >= PROTOCOL_VER_540) {
        if (lcm_on) {
            ...
            mp_test_run(chip_info, "short test -ili9881", testdata ); //compatible with old ini version.
            ...
            mp_test_run(chip_info, "noise peak to peak(ic only) (lcm off)", testdata);//ini not set
...
static int ili9881_black_screen_preoperation(char *msg, int msg_size, void *chip_data, struct auto_testdata *testdata)
{
    char apk_ret[100] = {0};
    struct ilitek_tddi_dev *chip_info = (struct ilitek_tddi_dev *)chip_data;

    chip_info->mp_result_count = 0;
    ilitek_tddi_mp_test_handler(apk_ret, NULL, msg, msg_size, OFF, chip_info, testdata);
    TPD_INFO("chip_info->mp_result_count = %d\n", chip_info->mp_result_count);
