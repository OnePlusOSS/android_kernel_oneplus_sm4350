...
static int host_download_dma_check(void *chip_data, u32 start_addr, u32 block_size, u32 *dma_crc)
{
    int count = 50;
    u32 reg_data = 0;
...
static int ilitek_tddi_fw_iram_read(void *chip_data, u8 *buf, u32 start, u32 end)
{
    int i;
...
static int ilitek_tddi_fw_iram_program(void *chip_data, u32 start, u32 size, u8 *w_buf)
{
    int ret = 0;
    u32 array[1];
    struct ilitek_tddi_dev *chip_info = (struct ilitek_tddi_dev *)chip_data;
    ...
    for (i = 0; i < ARRAY_SIZE(chip_info->fbi); i++) {
        if (chip_info->fbi[i].mode == mode && chip_info->fbi[i].len != 0) {
            TPD_INFO("Download %s code from hex 0x%x to IRAM 0x%x, len = 0x%x\n",
                     chip_info->fbi[i].name, chip_info->fbi[i].start, chip_info->fbi[i].mem_start, chip_info->fbi[i].len);


            ilitek_tddi_fw_iram_program(chip_info, chip_info->fbi[i].mem_start, chip_info->fbi[i].len, (fw_ptr + chip_info->fbi[i].start));

            crc = CalculateCRC32(chip_info->fbi[i].start, chip_info->fbi[i].len - 4, fw_ptr);
            dma = 0;
            ret = host_download_dma_check(chip_info, chip_info->fbi[i].mem_start, chip_info->fbi[i].len - 4, &dma);

            TPD_INFO("%s CRC is %s (%x) : (%x)\n",
                     chip_info->fbi[i].name, (crc != dma ? "Invalid !" : "Correct !"), crc, dma);

            if (ret < 0 || crc != dma) {
                ilitek_tddi_fw_print_iram_data(chip_info, chip_info->fbi[i].mem_start, chip_info->fbi[i].len);
                return UPDATE_FAIL;
            }
        }
    }
...
static void ilitek_tddi_fw_update_block_info(void *chip_data, u8 *pfw)
{
    ...
    if (tfd->hex_tag == BLOCK_TAG_AF) {
        chip_info->fbi[AP].mem_start = (chip_info->fbi[AP].fix_mem_start != INT_MAX) ? chip_info->fbi[AP].fix_mem_start : 0;
        chip_info->fbi[DATA].mem_start = (chip_info->fbi[DATA].fix_mem_start != INT_MAX) ? chip_info->fbi[DATA].fix_mem_start : DLM_START_ADDRESS;
        chip_info->fbi[TUNING].mem_start = (chip_info->fbi[TUNING].fix_mem_start != INT_MAX) ? chip_info->fbi[TUNING].fix_mem_start :  chip_info->fbi[DATA].mem_start + chip_info->fbi[DATA].len;
        chip_info->fbi[MP].mem_start = (chip_info->fbi[MP].fix_mem_start != INT_MAX) ? chip_info->fbi[MP].fix_mem_start :  0;
        ...
        chip_info->fbi[GESTURE].mem_start = (pfw[ges_info_addr + 7] << 24) + (pfw[ges_info_addr + 6] << 16) + (pfw[ges_info_addr + 5] << 8) + pfw[ges_info_addr + 4];
    ...
    /* Copy gesture data */
    if (chip_info->fbi[GESTURE].mem_start != 0xffffffff && ges_fw_start != 0xffffffff && chip_info->fbi[GESTURE].mem_start != 0 && ges_fw_start != 0)
    ...
    TPD_INFO("gesture memory start = 0x%x, upgrade lenth = 0x%x, hex area = %d, ges_start_addr = 0x%x, ges_end_addr = 0x%x",
             chip_info->fbi[GESTURE].mem_start, MAX_GESTURE_FIRMWARE_SIZE, ges_area_section, ges_fw_start, ges_fw_end);
    ...
    /* upgrade mode define */
    chip_info->fbi[DATA].mode = chip_info->fbi[AP].mode = chip_info->fbi[TUNING].mode = AP;
...
static int ilitek_tddi_fw_ili_convert(void *chip_data, u8 *pfw)
{
    ...
    if(!chip_info->p_firmware_headfile->firmware_data || chip_info->p_firmware_headfile->firmware_size <= ILI_FILE_HEADER
    ...
    for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
        if (((block_enable >> i) & 0x01) == 0x01) {
            num = i + 1;
            if ((num) == 6) {
                chip_info->fbi[num].start = (CTPM_FW[0] << 16) + (CTPM_FW[1] << 8) + (CTPM_FW[2]);
                chip_info->fbi[num].end = (CTPM_FW[3] << 16) + (CTPM_FW[4] << 8) + (CTPM_FW[5]);
                chip_info->fbi[num].fix_mem_start = INT_MAX;
            } else {
                chip_info->fbi[num].start = (CTPM_FW[34 + i * 6] << 16) + (CTPM_FW[35 + i * 6] << 8) + (CTPM_FW[36 + i * 6]);
                chip_info->fbi[num].end = (CTPM_FW[37 + i * 6] << 16) + (CTPM_FW[38 + i * 6] << 8) + (CTPM_FW[39 + i * 6]);
                chip_info->fbi[num].fix_mem_start = INT_MAX;
            }
            chip_info->fbi[num].len = chip_info->fbi[num].end - chip_info->fbi[num].start + 1;
            TPD_INFO("Block[%d]: start_addr = %x, end = %x\n", num, chip_info->fbi[num].start, chip_info->fbi[num].end);
        }
    }
...
out:
    ...
    tp_const_memcpy(pfw, MAX_HEX_FILE_SIZE,  CTPM_FW + ILI_FILE_HEADER,
                    ...
                    chip_info->p_firmware_headfile->firmware_size - ILI_FILE_HEADER);
    tfd->end_addr = (chip_info->p_firmware_headfile->firmware_size - ILI_FILE_HEADER);
    return ret;
}
...
static int ilitek_tddi_fw_hex_convert(void *chip_data, u8 *pfw)
{
    ...
    for (; i < size;) {
        ...
        if (type == 0x04) {
            ...
            chip_info->fbi[num].fix_mem_start = INT_MAX;
            chip_info->fbi[num].len = chip_info->fbi[num].end - chip_info->fbi[num].start + 1;
            TPD_INFO("Block[%d]: start_addr = %x, end = %x", num, chip_info->fbi[num].start, chip_info->fbi[num].end);
            ...
            TPD_INFO("Tag 0xB0: change Block[%d] to addr = 0x%x\n", num, chip_info->fbi[num].fix_mem_start);
        }
    ...
    for (i = 0; i < ARRAY_SIZE(chip_info->fbi); i++) {
        ...
        data_crc = CalculateCRC32(chip_info->fbi[i].start, chip_info->fbi[i].len - 4, pfw);
...
int ilitek_tddi_fw_upgrade(void *chip_data)
{
    ...
    if (chip_info->actual_tp_mode != P5_X_FW_GESTURE_MODE) {
        if ((ERR_ALLOC_MEM(chip_info->tp_firmware.data))
            ...
            tp_healthinfo_report(monitor_data, HEALTH_FW_UPDATE, "Open file fail, try upgrade from hex file");
    ...
    do {
        ...
        tp_healthinfo_report(monitor_data, HEALTH_FW_UPDATE, "Upgrade failed, do retry");
    ...
    if (ret != UPDATE_PASS) {
        ...
        ilitek_tddi_reset_ctrl(chip_info, ILITEK_RESET_METHOD);
        ret = UPDATE_FAIL;
        tp_healthinfo_report(monitor_data, HEALTH_FW_UPDATE, "Upgrade firmware failed after retry 3 times");
    }
    ...
    if (ret == UPDATE_PASS) {
        ...
        if(chip_info->debug_mode_sta && chip_info->actual_tp_mode != P5_X_FW_GESTURE_MODE) {
