    ...
    if (total_length > FIX_I2C_LENGTH) {
        if (ts->interface_data.write_buf_size < total_length) {
            if (ts->interface_data.write_buf) {
                tp_devm_kfree(&client->dev, (void **)&ts->interface_data.write_buf, ts->interface_data.write_buf_size);
                TPD_INFO("write block_1, free once.\n");
            }
            ts->interface_data.write_buf = tp_devm_kzalloc(&client->dev, total_length, GFP_KERNEL | GFP_DMA);
            if (!ts->interface_data.write_buf) {
                ts->interface_data.write_buf_size = 0;
                TPD_INFO("write block_1, kzalloc failed(len:%d, buf_size:%d).\n", total_length, ts->interface_data.write_buf_size);
        ...
        if (ts->interface_data.write_buf_size > FIX_I2C_LENGTH) {
            tp_devm_kfree(&client->dev, (void **)&ts->interface_data.write_buf, ts->interface_data.write_buf_size);
            ts->interface_data.write_buf = tp_devm_kzalloc(&client->dev, FIX_I2C_LENGTH, GFP_KERNEL | GFP_DMA);
            if (!ts->interface_data.write_buf) {
                ts->interface_data.write_buf_size = 0;
 ...
 */
inline int touch_i2c_read(struct i2c_client *client, char *writebuf, unsigned short writelen, char *readbuf, unsigned short readlen)
{
    int retval;
    unsigned char retry;
    ...
    if (writelen > FIX_I2C_LENGTH) {
        if (ts->interface_data.read_w_buf_size < writelen) {
            if (ts->interface_data.read_w_buffer) {
                tp_devm_kfree(&client->dev, (void **)&ts->interface_data.read_w_buffer, ts->interface_data.read_w_buf_size);
                TPD_INFO("read w block_1, free once.\n");
            }
            ts->interface_data.read_w_buffer = tp_devm_kzalloc(&client->dev, writelen, GFP_KERNEL | GFP_DMA);
            if (!ts->interface_data.read_w_buffer) {
                ts->interface_data.read_w_buf_size = 0;
                TPD_INFO("read w block_1, kzalloc failed(len:%d, buf_size:%d).\n", writelen, ts->interface_data.read_w_buf_size);
                mutex_unlock(&ts->interface_data.bus_mutex);
                return -ENOMEM;
            }
        ...
        if (ts->interface_data.read_w_buf_size > FIX_I2C_LENGTH) {
            tp_devm_kfree(&client->dev, (void **)&ts->interface_data.read_w_buffer, ts->interface_data.read_w_buf_size);
            ts->interface_data.read_w_buffer = tp_devm_kzalloc(&client->dev, FIX_I2C_LENGTH, GFP_KERNEL | GFP_DMA);
            if (!ts->interface_data.read_w_buffer) {
                ts->interface_data.read_w_buf_size = 0;
    ...
    if (readlen > FIX_I2C_LENGTH) {
        if (ts->interface_data.read_buf_size < readlen) {
            if (ts->interface_data.read_buf) {
                tp_devm_kfree(&client->dev, (void **)&ts->interface_data.read_buf, ts->interface_data.read_buf_size);
                TPD_INFO("read block_1, free once.\n");
            }
            ts->interface_data.read_buf = tp_devm_kzalloc(&client->dev, readlen, GFP_KERNEL | GFP_DMA);
            if (!ts->interface_data.read_buf) {
                ts->interface_data.read_buf_size = 0;
                TPD_INFO("read block_1, kzalloc failed(len:%d, buf_size:%d).\n", readlen, ts->interface_data.read_buf_size);
                mutex_unlock(&ts->interface_data.bus_mutex);
                return -ENOMEM;
            }
        ...
        if (ts->interface_data.read_buf_size > FIX_I2C_LENGTH) {
            tp_devm_kfree(&client->dev, (void **)&ts->interface_data.read_buf, ts->interface_data.read_buf_size);
            ts->interface_data.read_buf = tp_devm_kzalloc(&client->dev, FIX_I2C_LENGTH, GFP_KERNEL | GFP_DMA);
            if (!ts->interface_data.read_buf) {
                ts->interface_data.read_buf_size = 0;
 ...
 * Returning transfer msg length(transfer success) or most likely negative errno(transfer EIO error)
 */
inline int touch_i2c_write(struct i2c_client *client, char *writebuf, unsigned short writelen)
{
    int retval;
    u16 addr;
    struct touchpanel_data *ts = i2c_get_clientdata(client);
...
int32_t CTP_SPI_READ(struct spi_device *client, uint8_t *buf, uint16_t len)
{
	...
	if (ts->support_check_bus_suspend && ts->bus_ready == false) {
		pr_err("[TP]: %s bus_ready false\n", __func__);
		return -EINVAL;
	}
...
int spi_write_firmware(struct spi_device *client, u8 *fw, u32 *len_array, u8 array_len)
{
    ...
    t = tp_devm_kzalloc(&client->dev, sizeof(struct spi_transfer) * array_len, GFP_KERNEL | GFP_DMA);
    if (!t) {
