/*****************************************************************************************
 * Copyright (c)  2008- 2019  oplus Mobile communication Corp.ltd.
...
static void nvt_enable_edge_reject_limit(struct chip_data_nt36672c *chip_info);
	...
	struct auto_testdata *nvt_testdata, int limit_type)
{
	...
	if (ts_data == NULL || ts_data->hw_res == NULL || ts_data->hw_res->tx_num == 0 ||
		ts_data->hw_res->rx_num == 0 || nvt_testdata == NULL || nvt_testdata->pos == NULL) {
...
static void nvt_ts_read_history_log(void *chip_data)
{
	...
	nvt_set_page(chip_info, 0x23D00);
	buf[0] = 0x10;
	for (i = 1; i <65; i++) { buf[i]=0; }
	CTP_SPI_READ(chip_info->s_client, buf, 64);
	TPD_INFO("History1 Event:\n");
	for (i = 0; i <8; i++) {
		TPD_INFO("%02X %02X %02X %02X %02X %02X %02X %02X\n",
			buf[0+i*8], buf[1+i*8], buf[2+i*8], buf[3+i*8], buf[4+i*8], buf[5+i*8], buf[6+i*8], buf[7+i*8]);
	}
	buf[0] = 0x50;
	for (i = 1; i <65; i++) { buf[i]=0; }
	CTP_SPI_READ(chip_info->s_client, buf, 64);
	TPD_INFO("History2 Event:\n");
	for (i = 0; i <8; i++) {
		TPD_INFO("%02X %02X %02X %02X %02X %02X %02X %02X\n",
			buf[0+i*8], buf[1+i*8], buf[2+i*8], buf[3+i*8], buf[4+i*8], buf[5+i*8], buf[6+i*8], buf[7+i*8]);
	}
...
static int8_t nvt_edge_reject_parm_store(struct chip_data_nt36672c *chip_info,
	uint8_t u8Cmd, uint8_t u8SubCmd,
	uint16_t para1, uint16_t para2, uint16_t para3, uint16_t para4)
{
	int i, retry = 5;
	uint8_t buf[12] = {0};
	...
	for (i = 0; i < retry; i++) {
		...
		buf[9] = para4 & 0xFF;
		buf[10] = (para4 >> 8) & 0xFF;
		CTP_SPI_WRITE(chip_info->s_client, buf, 11);
		...
		buf[9] = 0xFF;
		buf[10] = 0xFF;
		CTP_SPI_READ(chip_info->s_client, buf, 11);
		if (buf[1] == 0x00)
			break;
	}

	TPD_DEBUG("send Cmd 0x%02X 0x%02X (%d %d %d %d), "
		"buf=0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X\n",
		u8Cmd, u8SubCmd, para1, para2, para3, para4,
...
static int nvt_get_gesture_info(void *chip_data, struct gesture_info *gesture)
{
	...
	case UP_VEE_DETECT:
		...
		gesture->Point_end.x   = (point_data[8] & 0xFF) | (point_data[9] & 0x0F) << 8;
		gesture->Point_end.y   = (point_data[10] & 0xFF) | (point_data[11] & 0x0F) << 8;
		gesture->Point_1st.x   = (point_data[12] & 0xFF) | (point_data[13] & 0x0F) << 8;
		gesture->Point_1st.y   = (point_data[14] & 0xFF) | (point_data[15] & 0x0F) << 8;
		break;
	...
	case CIRCLE_DETECT:
		...
		gesture->Point_start.x = (point_data[4] & 0xFF) | (point_data[5] & 0x0F) << 8;
		gesture->Point_start.y = (point_data[6] & 0xFF) | (point_data[7] & 0x0F) << 8;
		gesture->Point_end.x   = (point_data[8] & 0xFF) | (point_data[9] & 0x0F) <<
					 8;    /*ymin*/
		gesture->Point_end.y   = (point_data[10] & 0xFF) | (point_data[11] & 0x0F) << 8;
		gesture->Point_1st.x   = (point_data[12] & 0xFF) | (point_data[13] & 0x0F) <<
					 8;  /*xmin*/
		gesture->Point_1st.y   = (point_data[14] & 0xFF) | (point_data[15] & 0x0F) << 8;
		gesture->Point_2nd.x   = (point_data[16] & 0xFF) | (point_data[17] & 0x0F) <<
					 8;  /*ymax*/
		gesture->Point_2nd.y   = (point_data[18] & 0xFF) | (point_data[19] & 0x0F) << 8;
		gesture->Point_3rd.x   = (point_data[20] & 0xFF) | (point_data[21] & 0x0F) <<
					 8;  /*xmax*/
		gesture->Point_3rd.y   = (point_data[22] & 0xFF) | (point_data[23] & 0x0F) << 8;
		gesture->Point_4th.x   = (point_data[24] & 0xFF) | (point_data[25] & 0x0F) << 8;
		gesture->Point_4th.y   = (point_data[26] & 0xFF) | (point_data[27] & 0x0F) << 8;
		break;
	...
	case M_DETECT:
		...
		gesture->Point_2nd.x   = (point_data[16] & 0xFF) | (point_data[17] & 0x0F) << 8;
		gesture->Point_2nd.y   = (point_data[18] & 0xFF) | (point_data[19] & 0x0F) << 8;
		gesture->Point_3rd.x   = (point_data[20] & 0xFF) | (point_data[21] & 0x0F) << 8;
		gesture->Point_3rd.y   = (point_data[22] & 0xFF) | (point_data[23] & 0x0F) << 8;
		break;
...
static uint16_t EDGE_REGION_A = 10;
static uint16_t EDGE_REGION_B = 30;
static uint16_t EDGE_PURLICUE_X = 90;
static uint16_t EDGE_PURLICUE_Y = 90;
static uint16_t EDGE_CORNER_X = 120;
static uint16_t EDGE_CORNER_Y = 120;
static uint16_t EDGE_UD_CORNER_X = 120;
static uint16_t EDGE_UD_CORNER_Y = 120;

static void nvt_enable_edge_reject_limit(struct chip_data_nt36672c *chip_info)
{
	nvt_edge_reject_parm_store(chip_info, EVENTBUFFER_EXT_CMD, EVENTBUFFER_EXT_EDGE_PARM1,
		EDGE_REGION_A, EDGE_REGION_B, EDGE_PURLICUE_X, EDGE_PURLICUE_Y);

	nvt_edge_reject_parm_store(chip_info, EVENTBUFFER_EXT_CMD, EVENTBUFFER_EXT_EDGE_PARM2,
		EDGE_CORNER_X, EDGE_CORNER_Y, EDGE_UD_CORNER_X, EDGE_UD_CORNER_Y);
}
				 ...
				 int state)
{
	...
	nvt_enable_edge_reject_limit(chip_info);
				     ...
				     struct auto_testdata *nvt_testdata, struct test_item_info *p_test_item_info)
{
	...
	TPD_INFO("update test firmware successded!\n");
