...
static inline enum hrtimer_restart touch_up_timer_func(struct hrtimer *hrtimer, unsigned int i)
{
    struct touchpanel_data *ts = NULL;
    struct kernel_grip_info *grip_info = NULL;
    ...
    if (grip_info->work_id >= TOUCH_MAX_NUM)
        grip_info->work_id = 0;
    queue_work(ts->grip_info->grip_up_handle_wq, &ts->grip_info->grip_up_work[grip_info->work_id]);
    return HRTIMER_NORESTART;
}
...
struct kernel_grip_info *kernel_grip_init(struct device *dev)
{
    ...
    enum hrtimer_restart(*p_touch_up_timer_func[TOUCH_MAX_NUM]) (struct hrtimer * hrtimer) = {
