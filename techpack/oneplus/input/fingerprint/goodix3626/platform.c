...
int gf_power_on(struct gf_dev* gf_dev)
{
	...
	if (!vreg) {
		...
		vol = regulator_get_voltage(vreg);
...
int gf_power_off(struct gf_dev* gf_dev)
{
	...
	if (vreg) {
		...
		if (regulator_is_enabled(vreg)) {
			...
			pr_info("%s disabled fppower ret = %d\n", __func__, rc);
		}

		vol = regulator_get_voltage(vreg);
		pr_info("%s regulator_get_voltage %d next\n", __func__, vol);
