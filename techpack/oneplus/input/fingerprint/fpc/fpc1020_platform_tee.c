...
found:
	vreg = fpc1020->vreg[i];
	if (enable) {
		dev_err(dev, "vreg_setup enable %s\n", name);
		if (!vreg) {
		...
		if (regulator_count_voltages(vreg) > 0) {
			rc = regulator_set_voltage(vreg, vreg_conf[i].vmin,
					vreg_conf[i].vmax);
				dev_err(dev,
					"regulator_set_voltage set voltage on %s, min %d, max %d rc %d\n",
					name,  vreg_conf[i].vmin,  vreg_conf[i].vmax, rc);
			if (rc)
				dev_err(dev,
		...
		fpc1020->vreg[i] = vreg;
				...
				dev_err(dev, "vreg_setup disable %s\n", name);
		if (vreg) {
...
static DEVICE_ATTR(clk_enable, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH, NULL, clk_enable_set);
...
static DEVICE_ATTR(pinctl_set, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH, NULL, pinctl_set);
...
static DEVICE_ATTR(regulator_enable, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH, NULL, regulator_enable_set);
...
static DEVICE_ATTR(hw_reset, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH, NULL, hw_reset_set);
...
static DEVICE_ATTR(device_prepare, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH, NULL, device_prepare_set);
...
static DEVICE_ATTR(wakeup_enable, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH, NULL, wakeup_enable_set);
...
static DEVICE_ATTR(handle_wakelock, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH, NULL, handle_wakelock_cmd);
...
static DEVICE_ATTR(irq, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH, irq_get, irq_ack);
...
static DEVICE_ATTR(compatible_all, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH, NULL, compatible_all_set);
#endif
...
{
    int rc = 0;
	int fp_version = get_fp_version();
	pr_info("%s:fp version %x\n", __func__, fp_version);
	if (fp_version != 0x1020) {
		pr_info("%s id0 not fpc1020\n", __func__);
		return -ENOMEM;
	}
