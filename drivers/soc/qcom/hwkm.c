				       ...
				       size_t rsp_words)
{
	...
	// debug purpose: check bank ctl, bank status and IRQ status before transaction
	...
	// check ESR status before transaction
...
#ifdef HWKM_DEBUG_DUMP
	// debug purpose: check bank ctl, bank status and IRQ status after transaction
	val = qti_hwkm_readl(dev, QTI_HWKM_MASTER_RG_BANK2_BANKN_CTL, KM_MASTER);
	rmb();
	...
	pr_err("%s: after transaction - IRQ status %x in HWKM\n",
				__func__, val);
#endif
	//check ESR status, if error, following command error handling
	ESR_val = qti_hwkm_readl(dev, QTI_HWKM_MASTER_RG_BANK2_BANKN_ESR, KM_MASTER);
	rmb();
...
//if ESR error, clear the FIFO contents, ESR and exit out
Cmd_Err_Handling:
	if (err) {
		// Clear CMD FIFO: set CMD_FIFO_CLEAR_BIT to 1
		qti_hwkm_setb(dev, QTI_HWKM_MASTER_RG_BANK2_BANKN_CTL,
				CMD_FIFO_CLEAR_BIT, KM_MASTER);
		/* Write memory barrier */
		wmb();
...
{
	//clears tpkey receive block bit count and states
