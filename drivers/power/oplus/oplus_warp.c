...
EXPORT_SYMBOL(oplus_warp_switch_mode);
...
static int oplus_warp_set_current_1_temp_normal_range(struct oplus_warp_chip *chip, int vbat_temp_cur)
{
	...
	chip->warp_temp_cur_range = FASTCHG_TEMP_RANGE_NORMAL;
	...
	switch (chip->fastchg_batt_temp_status) {
		case BAT_TEMP_NATURAL:
			if (vbat_temp_cur > chip->warp_strategy1_batt_high_temp0) {
			...
			} else if (vbat_temp_cur > chip->warp_little_cool_temp) {
				...
				ret = chip->warp_normal_to_little_cool_current;
		...
		case BAT_TEMP_LOW0:
			if (vbat_temp_cur > chip->warp_strategy1_batt_high_temp0) {
				chip->fastchg_batt_temp_status = BAT_TEMP_HIGH0;
				ret = chip->warp_strategy1_high_current0;
			} else if (vbat_temp_cur < chip->warp_little_cool_temp) { //<16
				chip->fastchg_batt_temp_status = BAT_TEMP_LITTLE_COOL_LOW;
				ret = chip->warp_normal_to_little_cool_current;
				chip->warp_temp_cur_range = FASTCHG_TEMP_RANGE_LITTLE_COOL;
			} else {
	...
	switch (chip->fastchg_batt_temp_status) {
		...
		case BAT_TEMP_LITTLE_COOL_LOW:
			if (vbat_temp_cur < chip->warp_cool_temp) {
				...
				ret = chip->warp_strategy_normal_current;
				chip->warp_temp_cur_range = FASTCHG_TEMP_RANGE_COOL;
			} else if(vbat_temp_cur > chip->warp_little_cool_to_normal_temp){
				chip->fastchg_batt_temp_status = BAT_TEMP_NATURAL;
				ret = chip->warp_strategy_normal_current;
				chip->warp_temp_cur_range = FASTCHG_TEMP_RANGE_NORMAL;
			} else {
...
{
	int ret = 0;
	if (chip->warp_batt_over_low_temp != -EINVAL
...
out:
	...
	if (data == WARP_NOTIFY_NORMAL_TEMP_FULL || data == WARP_NOTIFY_BAD_CONNECTED) {
		...
		chip->fastchg_started = false;
		chip->fastchg_to_warm = false;
	} else if (data == WARP_NOTIFY_TEMP_OVER || oplus_warp_is_battemp_exit()) {
		usleep_range(350000, 350000);
		chip->fastchg_ing = false;
...
EXPORT_SYMBOL(oplus_warp_get_fastchg_started);
...
EXPORT_SYMBOL(oplus_warp_get_adapter_update_real_status);
...
EXPORT_SYMBOL(oplus_warp_get_btb_temp_over);
...
EXPORT_SYMBOL(oplus_warp_reset_fastchg_after_usbout);
...
EXPORT_SYMBOL(oplus_warp_reset_mcu);
...
EXPORT_SYMBOL(oplus_warp_get_fast_chg_type);
...
EXPORT_SYMBOL(oplus_warp_set_disable_adapter_output);
