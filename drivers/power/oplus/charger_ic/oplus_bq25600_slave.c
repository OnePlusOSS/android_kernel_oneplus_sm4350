...
static int __maybe_unused bq2560x_get_chargecurrent(struct bq2560x *bq, int *curr)
{
	u8 val;
	int ret;
...
static int __maybe_unused bq2560x_set_term_current(struct bq2560x *bq, int curr)
{
	u8 iterm;
...
static int __maybe_unused bq2560x_set_prechg_current(struct bq2560x *bq, int curr)
{
	u8 iprechg;
...
static int bq2560x_set_chargevoltage(struct bq2560x *bq, int volt)
{
	u8 val;
...
static int bq2560x_set_input_volt_ovp(struct bq2560x *bq, int ovp)
{
	u8 val;
...
static int __maybe_unused bq2560x_get_input_current_limit(struct bq2560x *bq, int *curr)
{
	u8 val;
	int ret;
...
static int __maybe_unused bq2560x_set_watchdog_timer(struct bq2560x *bq, u8 timeout)
{
	u8 val;
...
static int __maybe_unused bq2560x_reset_watchdog_timer(struct bq2560x *bq)
{
...
static int __maybe_unused bq2560x_reset_chip(struct bq2560x *bq)
{
	int ret;
...
static int bq2560x_init_parallel_psy(struct bq2560x *bq)
{
	struct power_supply_config parallel_cfg = {};

	parallel_cfg.drv_data = bq;
	parallel_cfg.of_node = bq->dev->of_node;
	parallel_cfg.num_supplicants = POWER_SUPPLY_TYPE_UNKNOWN;

	bq->parallel_psy_d.name	= "parallel";
	bq->parallel_psy_d.type	= POWER_SUPPLY_TYPE_MAINS;
	...
	if (IS_ERR(bq->parallel_psy)) {
		...
		return PTR_ERR(bq->parallel_psy);
	}
...
static int bq2560x_parse_dt(struct device *dev, struct bq2560x *bq)
{
	...
	if (bq->cfg.gpio_ce) {
		ret = gpio_request(bq->cfg.gpio_ce, "bq-ce");
		if (ret < 0)
			pr_err("request gpio bq-ce failed");

		ret = bq2560x_enable_hiz(bq);
		if (ret < 0)
			pr_err("enable bq2560x hiz failed");

		ret = gpio_direction_output(bq->cfg.gpio_ce, 1);
		if (ret < 0)
			pr_err("set gpio_ce high failed");
			   ...
			   const struct i2c_device_id *id)
{
	...
	ret = bq2560x_init_parallel_psy(bq);
	if (ret < 0) {
