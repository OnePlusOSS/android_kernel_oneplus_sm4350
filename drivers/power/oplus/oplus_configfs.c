                ...
                char *buf)
{
	...
	if(((oplus_warp_get_fastchg_started() == true) ||
		(oplus_warp_get_fastchg_dummy_started() == true))&&
		(chip->vbatt_num == 1)&&(oplus_warp_get_fast_chg_type() == FASTCHG_CHARGER_TYPE_UNKOWN)){
			type = CHARGER_SUBTYPE_FASTCHG_WARP;
	} else {
		type = oplus_warp_get_fast_chg_type();
		if (type == 0 && chip && chip->chg_ops->get_charger_subtype) {
			type = chip->chg_ops->get_charger_subtype();
		}
	}
...
int __attribute__((weak)) oplus_get_disconnect_vbus_status(void)
{
	return 0;
}

void __attribute__((weak)) oplus_set_disconnect_vbus_status(bool status)
{
	return;
}

static ssize_t connect_disable_show(struct device *dev, struct device_attribute
			 *attr, char *buf)
{
	int disconnect_vbus = 0;

	disconnect_vbus = oplus_get_disconnect_vbus_status();

	return sprintf(buf, "%d\n", disconnect_vbus);
}

static ssize_t connect_disable_store(struct device *dev, struct device_attribute *attr,
			const char *buf, size_t count)
{
	int val = 0;
	struct oplus_chg_chip *chip = NULL;
	...
	if (val == 1)
		oplus_set_disconnect_vbus_status(true);
	else
		oplus_set_disconnect_vbus_status(false);

	return count;
}

static DEVICE_ATTR_RW(connect_disable);

int __attribute__((weak)) oplus_get_con_temp1_status(void)
{
	return 0;
}

int __attribute__((weak)) oplus_get_con_temp2_status(void)
{
	return 0;
}


static ssize_t con_temp1_show(struct device *dev, struct device_attribute
                         *attr, char *buf)
{
        int con_temp1 = 0;

        con_temp1 = oplus_get_con_temp1_status();

        return sprintf(buf, "%d\n", con_temp1);
}
static DEVICE_ATTR_RO(con_temp1);

static ssize_t con_temp2_show(struct device *dev, struct device_attribute
                         *attr, char *buf)
{
        int con_temp2 = 0;

        con_temp2 = oplus_get_con_temp2_status();

        return sprintf(buf, "%d\n", con_temp2);
}
static DEVICE_ATTR_RO(con_temp2);
...
static ssize_t remaining_capacity_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	struct oplus_chg_chip *chip = NULL;
		...
		char *buf)
{
	...
	ret = chip->chg_ops->get_notify_code(chip->notify_code);
	return sprintf(buf, "%d\n", ret);
}
		...
		const char *buf, size_t count)
{
	...
	chip->chg_ops->set_shipmode(chip->enable_shipmode);
...
static ssize_t call_on_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t call_on_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	int val = 0;
	struct oplus_chg_chip *chip = NULL;
...
static DEVICE_ATTR_RW(call_on);

static ssize_t camera_on_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	struct oplus_chg_chip *chip = NULL;
...
static ssize_t camera_on_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	int val = 0;
	struct oplus_chg_chip *chip = NULL;
...
static DEVICE_ATTR_RW(camera_on);
