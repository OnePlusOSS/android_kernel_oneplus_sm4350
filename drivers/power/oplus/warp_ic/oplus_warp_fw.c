...
EXPORT_SYMBOL(get_warp_mcu_type);
...
void oplus_warp_fw_type_dt(struct oplus_warp_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,warp-little-cool-to-normal-temp",
			&chip->warp_little_cool_to_normal_temp);
	if (rc) {
		chip->warp_little_cool_to_normal_temp = 180;
	} else {
		chg_debug("qcom,warp-little-cool-to-normal-temp is %d\n", chip->warp_little_cool_to_normal_temp);
	}

	rc = of_property_read_u32(node, "qcom,warp-normal-to-little-cool-current",
			&chip->warp_normal_to_little_cool_current);
	if (rc) {
		chip->warp_normal_to_little_cool_current = 6;
	} else {
		chg_debug("qcom,warp-normal-to-little-cool-current is %d\n", chip->warp_normal_to_little_cool_current);
	}
